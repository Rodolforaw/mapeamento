...
function initializeMap() {
    ...
    // Evento de clique em obra específica - Mostrar informações salvas
    drawnItems.on('click', function(e) {
        const layer = e.layer;
        if (layer.workId) {
            // Mostrar popup com informações salvas
            showWorkInfoPopup(layer);
        }
    });
}
...
function initializeEventListeners() {
    ...
    document.getElementById('syncBtn').addEventListener('click', syncMapWithSupabase);
...
// Mostrar popup com informações da obra salva
function showWorkInfoPopup(layer) {
    // Criar popup personalizado com informações salvas
    const popupContent = `
        <div class="work-info-popup">
            <div class="work-info-header">
                <h3>${layer.workName || 'Obra sem nome'}</h3>
                <span class="work-type-badge">${getWorkTypeLabel(layer.workType)}</span>
            </div>
            ...
            <div class="work-info-body">
                <div class="info-row">
                    ...
                    <span class="value">${layer.workNumber || 'Não informado'}</span>
                </div>
                
                <div class="info-row">
                    <i class="fas fa-box"></i>
                    <span class="label">Produto:</span>
                    <span class="value">${layer.workProduct || 'Não informado'}</span>
                </div>
                
                <div class="info-row">
                    <i class="fas fa-ruler"></i>
                    <span class="label">Medida:</span>
                    <span class="value">${layer.workMeasure || 'Não informado'}</span>
                </div>
                
                <div class="info-row">
                    <i class="fas fa-comment"></i>
                    <span class="label">Observação:</span>
                    <span class="value">${layer.workObservation || 'Sem observação'}</span>
                </div>
                
                <div class="info-row">
                    <i class="fas fa-calendar"></i>
                    <span class="label">Data:</span>
                    <span class="value">${layer.workDate || 'Não informado'}</span>
                </div>
                
                <div class="info-row">
                    <i class="fas fa-info-circle"></i>
                    <span class="label">Status:</span>
                    <span class="value status-${layer.workStatus || 'planejamento'}">${getStatusLabel(layer.workStatus)}</span>
                </div>
            </div>
            
            <div class="work-info-actions">
                <button class="edit-btn" onclick="editWork('${layer.workId}')">
                    <i class="fas fa-edit"></i> Editar
                </button>
                <button class="delete-btn" onclick="deleteWork('${layer.workId}')">
    ...
    // Abrir popup no Leaflet
    layer.bindPopup(popupContent, {
        maxWidth: 400,
        className: 'custom-work-popup'
    }).openPopup();
}

// Obter label do tipo de obra
...
function getStatusLabel(status) {
    ...
    return labels[status] || 'Planejamento';
}

// Editar obra (chamada pelo popup)
function editWork(workId) {
    // Encontrar a obra pelo ID
    let targetWork = null;
    drawnItems.eachLayer(function(layer) {
        if (layer.workId === workId) {
            targetWork = layer;
        }
    });
    ...
    if (targetWork) {
        // Fechar popup
        targetWork.closePopup();
        
        // Atualizar obra atual para edição
        currentWork = targetWork;
        
        // Preencher modal com dados existentes
        document.getElementById('workNumber').value = targetWork.workNumber || '';
        document.getElementById('workProduct').value = targetWork.workProduct || '';
        document.getElementById('workMeasure').value = targetWork.workMeasure || '';
        document.getElementById('workObservation').value = targetWork.workObservation || '';
...
// Excluir obra (chamada pelo popup)
async function deleteWork(workId) {
    ...
    try {
        // Encontrar e remover a obra do mapa
        let targetWork = null;
        drawnItems.eachLayer(function(layer) {
            if (layer.workId === workId) {
                targetWork = layer;
        ...
        if (targetWork) {
            ...
            // Remover do mapa
            drawnItems.removeLayer(targetWork);
            
            // Excluir do Supabase
            const deleted = await deleteDrawingFromSupabase(workId);
            
            if (deleted) {
                showToast('Obra excluída com sucesso!', 'success');
            } else {
                showToast('Obra removida do mapa, mas erro ao excluir do banco', 'warning');
            }
        }
    } catch (error) {
