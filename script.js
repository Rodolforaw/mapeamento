...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Debug: Verificar se as obras estão sendo exibidas
    setTimeout(() => {
        const layers = drawnItems.getLayers();
        console.log(`DEBUG: Total de obras no mapa: ${layers.length}`);
        layers.forEach((layer, index) => {
            console.log(`DEBUG: Obra ${index + 1}:`, layer.workName, layer.workId);
        });
    }, 2000);
});
        ...
        if (data && data.length > 0) {
            ...
            data.forEach(workData => {
                ...
                if (work) {
                    ...
                    console.log('Obra adicionada ao mapa:', work.workName);
                } else {
...
function createWorkFromData(workData) {
    try {
        ...
        console.log('Obra criada e adicionada ao mapa:', work.workName);
        return work;
    } catch (error) {
