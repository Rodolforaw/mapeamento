...
function syncCrossContextData() {
    try {
        console.log('🔄 Tentando sincronizar dados entre contextos...');
        ...
        // Estratégia 1: Verificar se há dados no Supabase para sincronizar
        if (window.supabaseConfig && window.supabaseConfig.loadMarkings) {
            console.log('📡 Tentando carregar dados do Supabase...');
            window.supabaseConfig.loadMarkings().then(result => {
                if (result.success && result.markings.length > 0) {
                    ...
                    showNotification(`🔄 Sincronizados ${mergedMarkings.length} marcações do Supabase`, 'success');
                } else {
                    console.log('ℹ️ Nenhuma marcação encontrada no Supabase');
                    // Estratégia 2: Tentar sincronização via servidor local
                    tryLocalServerSync();
                }
            }).catch(error => {
            ...
            console.log('⚠️ Supabase não disponível, tentando servidor local...');
            // Estratégia 2: Tentar sincronização via servidor local
            tryLocalServerSync();
        }
...
// Mesclar marcações evitando duplicatas
function mergeMarkings(localMarkings, remoteMarkings) {
    const merged = [...localMarkings];
    const existingIds = new Set(localMarkings.map(m => m.id));
    
    remoteMarkings.forEach(marking => {
...
function setupRealTimeSync() {
    ...
    window.addEventListener('markingsUpdated', function(e) {
        setTimeout(() => {
            refreshWorksTable();
            // Auto-sincronizar com servidor quando houver mudanças
    ...
    // Polling para verificar mudanças no servidor a cada 30 segundos
    setInterval(() => {
        if (isOnline && !syncInProgress) {
            loadDataFromServer(true); // Sincronização silenciosa
        }
    }, 30000);
