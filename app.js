...
function saveMarking(layer, type) {
    ...
    if (isPWA) {
        // No PWA, tentar popup primeiro, se falhar salvar diretamente
        try {
            showWorkInfoPopup(layer, type);
        } catch (error) {
            console.error('Erro no popup PWA, salvando diretamente:', error);
            saveMarkingDirect(layer, type);
        }
    } else {
        // No PC, mostrar popup para informações da obra
        showWorkInfoPopup(layer, type);
    }
}

// Salvar marcação diretamente (PWA)
function saveMarkingDirect(layer, type) {
    const marking = {
        id: generateId(),
        type: type,
    ...
    // Adicionar popup básico
    const popupContent = `
        <div style="min-width: 150px;">
            <h4>📋 ${marking.properties.name}</h4>
            <p><strong>Tipo:</strong> ${type}</p>
            <p><strong>Criado:</strong> ${new Date(marking.timestamp).toLocaleString('pt-BR')}</p>
        </div>
    `;
...
function showWorkInfoPopup(layer, type) {
    ...
    const existingModal = document.querySelector('.work-modal');
    if (existingModal) {
    ...
    // Timeout de segurança para PWA
    const isPWA = isPWAMode();
    if (isPWA) {
        setTimeout(() => {
            if (document.body.contains(modal)) {
                console.log('Timeout PWA - salvando diretamente');
                ...
                try {
                    ...
                    console.error('Erro ao remover modal no timeout:', error);
                }
            }
...
async function exportKMZ() {
    ...
    if (typeof JSZip === 'undefined') {
        showNotification('Biblioteca JSZip não carregada. Exportando como KML...', 'info');
        exportKML(markings);
        return;
    }
    ...
    try {
        ...
        showNotification('Erro ao exportar KMZ. Tentando KML...', 'info');
        exportKML(markings);
    }
}

// Exportar KML (fallback)
function exportKML(markings) {
    try {
        const kml = generateKML(markings);
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `controle_obra_${new Date().toISOString().split('T')[0]}.kml`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
...
async function processKMZFile(file) {
    // Verificar se JSZip está disponível
    if (typeof JSZip === 'undefined') {
        showNotification('Biblioteca JSZip não carregada. Processando como KML...', 'info');
    ...
    try {
        ...
        // Processar KML
        await processKMLContent(kmlContent);
        
    } catch (error) {
        console.error('Erro ao processar KMZ:', error);
        showNotification('Erro ao processar KMZ. Tentando como KML...', 'info');
        await processKMLFile(file);
    }
}
...
async function processKMLContent(kmlContent) {
    try {
        // Verificar se xml2js está disponível
        if (typeof xml2js === 'undefined') {
            showNotification('Biblioteca xml2js não carregada. Usando parser simples...', 'info');
            const markings = parseKMLSimple(kmlContent);
            await saveImportedMarkings(markings);
            return;
        }
        
        // Parsear KML com xml2js
...
// Parser KML simples (fallback)
function parseKMLSimple(kmlContent) {
    const markings = [];
    
    try {
        // Extrair Placemarks usando regex
        ...
        while ((match = placemarkRegex.exec(kmlContent)) !== null) {
            ...
            const marking = parsePlacemarkSimple(placemarkContent, index++);
            if (marking) {
                markings.push(marking);
            }
        }
    } catch (error) {
        console.error('Erro no parser KML simples:', error);
    }
    
    return markings;
}

// Parsear Placemark simples
function parsePlacemarkSimple(placemarkContent, index) {
    try {
        // Extrair nome
        const nameMatch = placemarkContent.match(/<name>([^<]*)<\/name>/);
        const name = nameMatch ? nameMatch[1] : `Importada ${index + 1}`;
        ...
        // Determinar tipo e coordenadas
        if (placemarkContent.includes('<Point>')) {
            const coordsMatch = placemarkContent.match(/<coordinates>([^<]*)<\/coordinates>/);
            if (coordsMatch) {
        ...
        } else if (placemarkContent.includes('<Polygon>')) {
            const coordsMatch = placemarkContent.match(/<coordinates>([^<]*)<\/coordinates>/);
            if (coordsMatch) {
                const coords = coordsMatch[1].trim().split(' ').map(coord => {
        ...
        } else if (placemarkContent.includes('<LineString>')) {
            const coordsMatch = placemarkContent.match(/<coordinates>([^<]*)<\/coordinates>/);
            if (coordsMatch) {
                const coords = coordsMatch[1].trim().split(' ').map(coord => {
                    const parts = coord.split(',');
        ...
        console.error('Erro ao parsear Placemark:', error);
    }
    
    return null;
}

// Salvar marcações importadas
async function saveImportedMarkings(markings) {
    if (markings.length === 0) {
