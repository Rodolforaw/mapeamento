...
`;
    ...
    drawnItems.eachLayer(function(layer) {
        kml += `<Placemark>
`;
        kml += `<name>Marcação ${placemarkIndex}</name>
`;
        kml += `<description>Criado em: ${new Date().toLocaleString('pt-BR')}</description>
`;
...
`;
            kml += `<description>Círculo - Raio: ${radius.toFixed(2)}m</description>
`;
        }
...
function updateConnectionStatus() {
    ...
    if (!statusIndicator) {
        ...
        controlPanel.insertBefore(statusIndicator, controlPanel.firstChild);
    }
...
function setupWorksManagementEventListeners() {
    ...
    const clearWorksBtn = document.getElementById('clear-works-btn');
    const osFilterInput = document.getElementById('os-filter');
...
function refreshWorksTable() {
    ...
    const summaryElement = document.getElementById('works-summary');
    
    if (!tbody || !summaryElement) return;
    ...
    // Atualizar resumo
    summaryElement.textContent = `${osNumbers.length} obra(s) • ${totalMarkings} marcação(ões)`;
}

// Filtrar tabela de obras
function filterWorksTable() {
    const filter = document.getElementById('os-filter').value.toLowerCase();
...
function downloadWorkExcel(osNumber) {
    ...
    try {
        ...
        const data = work.markings.map((marking, index) => ({
            ...
            'Tipo': getLayerTypeName(marking.type),
