    ...
    try {
        ...
        markings.forEach(marking => {
            try {
                ...
                console.warn(`⚠️ Erro ao processar marcação ${marking.id}:`, error);
                // Continuar com a próxima marcação em vez de quebrar tudo
            }
        });
    ...
    try {
        ...
        if (marking.type === 'marker') {
            ...
            if (marking.coordinates && marking.coordinates.lng !== undefined && marking.coordinates.lat !== undefined) {
                ...
                console.warn('Coordenadas inválidas para círculo:', marking.coordinates);
                return null;
            }
        }
...
function setupRealTimeSync() {
    ...
    window.addEventListener('markingsUpdated', function(e) {
        setTimeout(() => {
            refreshWorksTable();
            // Sincronização automática removida - apenas manual via botão
        }, 100);
    });
    ...
    // Polling local removido - apenas sincronização manual via botão
}
